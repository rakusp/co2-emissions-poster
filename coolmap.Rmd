...
prepareDataForPlot <- function(mapData){
  ...
  worldMap$long <- worldMap$long+3#+runif(length(worldMap$long), min = -1, max = 1)*4
  worldMap$lat <- worldMap$lat-3#+runif(length(worldMap$lat), min = -1, max = 1)*4
...
dataForMap <-prepareDataForPlot(prepareDataEdges(export,2014,1000, c("USA", "Poland", "China", "Australia", "United Arab Emirates", "Brazil", "South Africa", "Norway")))
```


```{r}
# Plot
print(max(dataForMap$size))
...
p <- ggplot() +
  ...
  geom_map(data=dataForMap,aes(map_id=countryname, alpha =1, color=countryname), fill = "gray", map = worldMap, size=2) +
  geom_line(data=dataForMap, aes(x=lon, y=lat, group=group, color=countryname, lwd=5*size), alpha=1) +
